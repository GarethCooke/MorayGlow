...
//   3. If no credentials or connection fails → starts "MorayGlow-Setup" AP.
...
void setupNetwork() {
    ...
    if (cfg.isValid() && cfg.hasSSID()) {
        ...
        if (WiFi.status() == WL_CONNECTED) {
            ...
            // Register unique mDNS hostname (morayglow-a1b2c3.local)
    ...
    apMode       = true;
    String ssid  = Device::apSsid();  // e.g. "MorayGlow-A1B2C3"
    WiFi.mode(WIFI_AP);
